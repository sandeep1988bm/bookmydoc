...
public class Doctor {
	...
	public String getpassword(int doc_id) throws SQLException,
		...
		while (rs.next()) {
			password1 = rs.getString("d_password");
			return password1;
		}
		obj.createConn();
		return password1;
	...
	public void setpassword(String newpwd) throws SQLException,
			ClassNotFoundException {
		obj.createConn();
		String query = "Update doctor set d_password=" + newpwd + "where id="
				+ id;
	...
	public boolean login(int chk_id, String chk_password) throws Exception {
		obj.createConn();
		String query = "Select * from doctor where id=" + chk_id;
	...
	public int register(int pin1, String password1, int age1, String name1,
		...
		if (obj.getRowCount(pinvalidate) == 0) {
			...
			// id=id1;
			String updatequery = "update doctor set id=" + id1 + ",d_name='"
					+ name1 + "',age=" + age1 + ",d_password='" + password1
					+ "',d_speciality='" + type1 + "'where pin=" + pin1;
